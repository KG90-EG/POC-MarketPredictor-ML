"""Prometheus configuration for scraping ML metrics."""

PROMETHEUS_CONFIG = """
# Prometheus configuration for ML Trading Predictor
global:
  scrape_interval: 15s
  evaluation_interval: 15s
  external_labels:
    monitor: 'ml-trading-monitor'
...
def save_prometheus_config(config_dir: str = "./monitoring/prometheus"):
    ...
    # Save main config
    with open(config_path / "prometheus.yml", "w") as f:
        f.write(PROMETHEUS_CONFIG)

    # Save alert rules
    with open(config_path / "alert_rules.yml", "w") as f:
        f.write(ALERT_RULES)

    print(f"Prometheus configuration saved to {config_dir}")
