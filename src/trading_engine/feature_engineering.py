...
def compute_parabolic_sar(high: pd.Series, low: pd.Series, close: pd.Series, 
...
def compute_keltner_channels(close: pd.Series, high: pd.Series, low: pd.Series, 
...
def add_all_features(df: pd.DataFrame, ticker: str = None) -> pd.DataFrame:
    ...
    if ticker:
        ...
        for key in ['pe_ratio', 'peg_ratio', 'roe', 'debt_to_equity', 'profit_margin',
                    'operating_margin', 'eps_growth', 'revenue_growth', 'free_cash_flow', 'dividend_yield']:
            df[key] = np.nan
...
def get_feature_names() -> List[str]:
    ...
    return [
        # Original 9
        'SMA50', 'SMA200', 'RSI', 'Volatility', 'Momentum_10d', 'MACD', 'MACD_signal', 'BB_upper', 'BB_lower',
        
        # Advanced Technical (11)
        'ATR', 'ADX', 'Stochastic_K', 'Stochastic_D', 'OBV', 'VWAP', 'Williams_R', 'CCI',
        'Parabolic_SAR', 'Ichimoku_Conversion', 'Keltner_Middle',
        
        # Fundamentals (10)
        'pe_ratio', 'peg_ratio', 'roe', 'debt_to_equity', 'profit_margin', 'operating_margin',
        'eps_growth', 'revenue_growth', 'free_cash_flow', 'dividend_yield',
        
        # Sentiment (5)
        'analyst_rating', 'institutional_ownership', 'insider_ownership', 'short_ratio', 'news_sentiment',
        
        # Macro (5)
        'vix', 'treasury_yield', 'usd_index', 'market_breadth', 'sector_performance',
    ]
