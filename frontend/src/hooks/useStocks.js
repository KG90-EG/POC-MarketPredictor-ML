...
export function useStockRanking(initialViews = ['Global']) {
  ...
  const fetchRanking = useCallback(async (views = selectedViews) => {
    setLoading(true)
    setError(null)
...
export function useTickerSearch() {
  ...
  const performSearch = useCallback(async (ticker = searchTicker) => {
  ...
  useEffect(() => {
    const checkHealth = async () => {
      try {
        ...
        const allHealthy = healthResp.api_healthy && 
                          healthResp.model_loaded && 
                          healthResp.openai_configured
