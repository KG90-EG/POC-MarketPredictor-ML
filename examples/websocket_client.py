...
async def test_websocket():
    ...
    try:
        async with websockets.connect(uri) as websocket:
            ...
            # Subscribe to AAPL
            print("\nSubscribing to AAPL...")
            await websocket.send(json.dumps({"action": "subscribe", "ticker": "AAPL"}))

            # Subscribe to MSFT
            print("Subscribing to MSFT...")
            await websocket.send(json.dumps({"action": "subscribe", "ticker": "MSFT"}))

            # Subscribe to NVDA
            print("Subscribing to NVDA...")
            await websocket.send(json.dumps({"action": "subscribe", "ticker": "NVDA"}))
            ...
            while True:
                try:
                    ...
                    if data["type"] == "subscribed":
                        ...
                        # Color-coded output
                        color = (
                            "\033[92m" if change >= 0 else "\033[91m"
                        )  # Green or Red
                        reset = "\033[0m"

                        print(
                            f"[{message_count}] {ticker:5} ${price:8.2f} "
